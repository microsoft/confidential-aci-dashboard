...
for file in *.html; do
    if [ "$file" != "index.html" ]; then
        ...
        echo "<div data-workload=\"$workload\" data-region=\"$region\" class=\"graph\">$html_content</div>" >> "$temp_file"
...
# Append graphs, workload buttons, and region buttons to the template file
awk -v insert="$(cat $temp_file)" -v workload_buttons="$workload_buttons" -v region_buttons="$region_buttons" '
    /<div id="graphs"/ { print; inside_graphs_div=1; next }
    inside_graphs_div && /<\/div>/ { print insert; inside_graphs_div=0 }
